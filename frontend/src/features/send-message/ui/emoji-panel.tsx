...
/**
 * Emoji section of the composer's media picker: search, category tabs, and a
 * frequently-used row. The dataset is a curated Unicode subset (see
 * model/emoji-data.ts), not the full emoji catalog.
 */
export function EmojiPanel({ recentEmojis, onPick }: EmojiPanelProps) {
          ...
          <div className="flex items-center gap-0.5 border-t border-border pt-1.5">
            {EMOJI_CATEGORIES.map((group) => (
              <Button
                key={group.id}
                size="icon"
                // An active icon button reads the same everywhere in the
                // app: the secondary surface, never a hand-rolled tint.
                variant={group.id === activeCategory ? "secondary" : "ghost"}
