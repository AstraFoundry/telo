...
- **`main.ts`** — Electron main-process entry point. Builds the container, registers IPC handlers, hides Chromium's application menu and the native title bar (Window Controls Overlay keeps min/max/close), creates the window, and pushes Telegram auth-state events to the renderer.
