...
- `preferences.json`: user preferences (agent panel visibility, demo workspace opt-in, interface theme, accent color, message text size, time format, send-with-Enter, notifications toggle). It holds no secrets, so it is plain JSON with a restrictive file mode rather than `safeStorage` encryption. Files written by older builds may lack newer fields; the domain normalizer falls back to each field's default instead of rejecting the file.
