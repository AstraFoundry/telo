...
- `dialogs.json`: a snapshot of the chat list and folder badges (no avatars, no secrets). On launch the workspace hydrates from this file while the Telegram session is restoring, then refreshes with a paged `messages.GetDialogs`. Folder unread is computed from the snapshot rather than a second full dialog walk. Logout deletes the file.
