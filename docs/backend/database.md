...
- `preferences.json`: user preferences (agent panel visibility, demo workspace flag, interface theme, accent color, message text size, chat background, time format, send-with-Enter, notifications toggle, per-chat-kind message notifications, notification sound, per-kind media auto-download, media cache ceiling). The demo flag is written on process start from `TELO_DEMO_WORKSPACE=1` (`make dev DEMO=1`); it is not an in-app opt-in. The file holds no secrets, so the file is plain JSON with a restrictive file mode rather than `safeStorage` encryption. Files written by older builds may lack newer fields; the domain normalizer falls back to each field's default instead of rejecting the file.
