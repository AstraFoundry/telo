...
- The loading state (auth initializing) renders `copy.connectionConnecting` inside the beui `TextShimmer`, wrapped in a `role="status"` paragraph. The action area keeps a fixed `min-h-11` and crossfades (opacity, 200 ms `EASE_OUT`, `AnimatePresence mode="wait"`) between the shimmer and the `copy.startMessaging` button so the swap never teleports. `TextShimmer` injects its keyframes via a `<style>` tag, so assertions must use `toContain` on the status text.
