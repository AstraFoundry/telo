...
| Unread divider      | `ChatDto.lastReadMessageId` (teleproto `readInboxMaxId`; deterministic in the demo workspace) places an accent `MessageMarker` before the first unread message; if the boundary message is not loaded, older pages load until found or the cursor is exhausted                                                                                                        | `demo-telegram-repository.test.ts`, `conversation-view.test.tsx`, `messaging-sync.spec.ts`                                                                       |
