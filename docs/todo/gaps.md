...
- [x] Agent 面板 C 端化整治 — 去掉 assistant 头像、scope 分段选择器、Payload preview、Recent runs（IPC 保留，仅不再渲染）；scope 由 `buildScopeInput` 隐式推导（附加消息 → selected，当前会话 → unread，否则 folder）；多选栏新增 `features/add-to-agent`「Add to Agent」，选中消息以小卡片进入 `PromptInput` 附件槽并随下一次 run 消费；Summarize / Extract 变为 composer 顶部 pill；输出前 `ResponseSkeleton` 占位、Streamdown 增量渲染 Markdown；`RUN_ERROR` 不再落入 transcript，改为一行静默失败 + 重试（`lastRun`）；后端 `TeleprotoTelegramRepository` 吸收 ≤30s `FLOOD_WAIT` 重试，`AgentContextService.findChat` 找到即停翻页；e2e `agent-context.spec.ts` 改覆盖 folder 隐式 scope 与 Add to Agent → selected 路径
- [x] Agent 面板第二轮：动态推荐问 / 引用 / mention — 推荐问改为模型生成：`AgentGateway.suggest`（`generateText` 出 JSON 数组，`parseSuggestions` 宽松解析）+ `RunAgentService.suggestFollowUps`，IPC 在 `RUN_FINISHED` 之后追加 `CUSTOM` `suggestions` 事件，失败只记日志；pill 缩为 28px 无边框半透明、`useScrollFade` 遮罩渐隐、transcript 上翻时条右端出「回到最新」按钮。引用不再是 `[[telo-cite]]` + 编号 button，载荷行改为 `ref: telo://message/<chatId>/<messageId>` + `AGENT_REFERENCE_INSTRUCTION`，`parseReply` 把回复切成 text / citation / mention 段，citation 渲染为内联上标 `<a href="telo://…">`，`app/telo-links.ts` 全局委托点击 → `requestJumpToMessage`（历史线程、folder 跨 chat 均可跳）。`@Name` 由 `buildMentionTargets`（消息作者 + `listChatMembers` + chat 列表）解析成头像 + 名字的内联 chip；输入框 `useMentionPicker` 复用抽到 `shared/ui` 的 `MentionAutocomplete`（`OptionRow` 新增 `leading` 头像槽，`ChatMemberDto` 补 `avatarDataUrl`，群聊 composer 同步受益），支持带空格的 display name
