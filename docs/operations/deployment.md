...
Every pull request runs the complete quality gate on Ubuntu under Xvfb. A tag matching `v*` starts a matrix build on macOS, Windows, and Ubuntu. Electron Builder creates native artifacts and GitHub Actions uploads them to the GitHub release associated with the tag.
