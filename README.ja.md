...
- **細やかなカスタマイズ**：
  ...
  - チャット背景：**プレーン**、**ドット**、**グリッド**、**グラデーション**。すべてテーマトークンから描画。
  - 時刻表示形式：**システム標準**、**12 時間制**、**24 時間制**。
  - 送信ショートカット設定：`Enter` 送信または `Cmd/Ctrl + Enter` 送信。
  - ショートカット操作：`Cmd/Ctrl + B` で AI エージェントパネルを瞬時に開閉。
- **答えを返す設定画面**：
  - 検索はセクションではなく設定項目を引く：別名で入力し、矢印キーで選び、`Enter` でその行に着地。着地した行は一度だけ光る。
  - 説明文の代わりにライブプレビュー：外観は会話と同じコンポーネントとトークンで実際の吹き出しを描画し、通知は OS が出す横幅そのものを描画。
  - チャット種別ごとの通知制御（**プライベート**、**グループ**、**チャンネル**）に加え、送信者名・本文・サウンドの切り替え。
  - メディア種別ごとの自動ダウンロード（**写真**、**動画と GIF**、**ファイル**）と、ディスクキャッシュ上限。
...
- **アニメーション絵文字**：絵文字 1 文字だけのメッセージは Telegram 公式のアニメーションを一度だけ再生して静止。クリックすると Telegram に特大エフェクトを要求し、会話の上に重ねて再生する。相手がクリックしたときも同じエフェクトがこちらで再生される。アニメーションのない絵文字だけのテキストは、Telegram と同じ 3 つまでの上限で大きく描画。
